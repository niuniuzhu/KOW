...
export class HPPacket {
	...
	public Update(dt: number): void {
		while (this._time >= this._refreshInterval) {
			...
			const rndx = this._battle.random.NextDouble(this._battle.bounds.xMin + this._padding, this._battle.bounds.xMax - this._padding);
			const rndy = this._battle.random.NextDouble(this._battle.bounds.yMin + this._padding, this._battle.bounds.yMax - this._padding);
