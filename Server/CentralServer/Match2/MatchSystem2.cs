	...
	public class MatchSystem2
	{
		...
		public bool Join( RoomUser user )
		{
			...
			if ( !room.AddUser( user ) )
				return false;
			this.eventHandler( MatchEvent.Type.AddToRoom, user, room.GetRoomInfo() );
			if ( room.isFull )
			{
				this.eventHandler( MatchEvent.Type.MatchSuccess, null, room.GetRoomInfo() );
		...
		public bool Leave( RoomUser user )
		{
			...
			if ( !room.RemoveUser( user ) )
				return false;
			this.eventHandler( MatchEvent.Type.RemoveFromRoom, user, room.GetRoomInfo() );
			if ( room.isEmpty )
			{
				this._rooms.Remove( room );
				RoomPool.Push( room );
		...
		public void Update( long dt )
		{
			...
			if ( this._checkRoomTime >= this._checkRoomInterval )
			{
				this.CheckRoom();
				this._checkRoomTime = 0;
			}
			else
				this._checkRoomTime += dt;
		}
				...
				for ( int j = i + 1; j < this._rooms.Count; j++ )
				{
					...
					for ( int k = 0; k < nxt.numUsers; ++k )
					{
							 ...
							 user.rank < cur.to )
						{
							...
							this.eventHandler( MatchEvent.Type.RemoveFromRoom, user, nxt.GetRoomInfo() );
							System.Diagnostics.Debug.Assert( cur.AddUser( user ) );
							this.eventHandler( MatchEvent.Type.AddToRoom, user, cur.GetRoomInfo() );
					...
					if ( cur.isFull )
					{
						this.eventHandler( MatchEvent.Type.MatchSuccess, null, cur.GetRoomInfo() );
