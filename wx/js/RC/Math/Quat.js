...
export class Quat {
    ...
    ToAngleAxis() {
        ...
        Vec3.MulN(result[1], MathUtils.RAD_TO_DEG);
        return result;
    }
    ...
    static ToEulerRad(rotation) {
        ...
        v.y = MathUtils.Atan2(2 * q.x * q.w + 2 * q.y * q.z, 1 - 2 * (q.z * q.z + q.w * q.w));
        v.x = MathUtils.Asin(2 * (q.x * q.z - q.w * q.y));
        v.z = MathUtils.Atan2(2 * q.x * q.y + 2 * q.z * q.w, 1 - 2 * (q.y * q.y + q.z * q.z));
        return Quat.NormalizeAngles(Vec3.MulN(v, MathUtils.RAD_TO_DEG));
    }
    ...
    static ToAxisAngleRad(q) {
        if (MathUtils.Abs(q.w) > 1.0)
            q = Quat.Normalize(q);
        let angle = 2.0 * MathUtils.Acos(q.w);
